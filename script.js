(() => {
    ...
    function updateCrosshair() {
        ...
        console.log('Current background:', backgrounds[currentBackgroundIndex]);
        console.log('Image loaded:', currentImage.complete);
    ...
    function drawCrosshair(config, canvas, isDeployed = false) {
        ...
        const outlineOffset = { xy: 0.5 * outlineThickness, wh: 1 * outlineThickness };
        ...
        if (config.style !== 2 || isDeployed) {
            ...
            ctx.globalAlpha = originalAlpha * (1 - Math.pow(1 - config.outerSplitAlpha, 0.45));
            drawCrosshairPart(ctx, startX + thickness + gap + splitLength + 1, startY, length, thickness, outlineOffset, outlines);
            drawCrosshairPart(ctx, startX - gap - length - splitLength - 1, startY, length, thickness, outlineOffset, outlines);
            if (!config.tStyleEnabled) drawCrosshairPart(ctx, startX, startY - gap - length - splitLength - 1, thickness, length, outlineOffset, outlines);
            drawCrosshairPart(ctx, startX, startY + thickness + gap + splitLength + 1, thickness, length, outlineOffset, outlines);
            ctx.globalAlpha = originalAlpha * (1 - Math.pow(1 - config.innerSplitAlpha, 0.45));
            length = splitLength;
        ...
        if (!config.tStyleEnabled) drawCrosshairPart(ctx, startX, startY - gap - length, thickness, length, outlineOffset, outlines);
        drawCrosshairPart(ctx, startX, startY + thickness + gap, thickness, length, outlineOffset, outlines);
        if (config.centerDotEnabled) drawCrosshairPart(ctx, startX, startY, thickness, thickness, outlineOffset, outlines);
    ...
    function drawCrosshairPart(ctx, x, y, width, height, outlineOffset, outlines) {
    ...
    function preloadImages() {
        backgrounds.forEach((bg, index) => {
            ...
            backgroundImages[index].onload = () => console.log(`Image loaded: ${bg.name}`);
            backgroundImages[index].onerror = () => console.error(`Failed to load image: ${bg.url}`);
    ...
    document.getElementById('zoomSliderValue').addEventListener('keydown', (e) => {
        if (e.key === 'Enter') {
            e.preventDefault();
            e.target.blur();
        }
